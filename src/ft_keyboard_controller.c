...
void    ft_turnleft(t_win *game)
{
    ...
    game->player->dirx = game->player->dirx * cosinus - game->player->diry * sinus;
    game->player->diry = game->player->olddirx * sinus + game->player->diry * cosinus;
    game->player->oldplanex = game->player->planex;
    game->player->planex = game->player->planex * cosinus - game->player->planey * sinus;
    game->player->planey =  game->player->oldplanex * sinus + game->player->planey * cosinus;
...
void    ft_new_event_player(t_win *c_ct)
{
     if (SDL_PollEvent(&c_ct->event)) {
         ...
         (c_ct->event.type == SDL_QUIT ||  c_ct->currentKeyStates[SDL_SCANCODE_ESCAPE]) ? (c_ct->statement = 0) : 0;
         if (c_ct->event.type == SDL_KEYDOWN) {
             ft_music_player_keyevent_down(c_ct);
             if ((c_ct->music.play == 1) || (c_ct->music.pause == 1) || (c_ct->music.stop == 1) ||
...
void    ft_event_manu(t_win *c_ct)
{
    ...
    {
        (c_ct->mpthree.type == SDL_QUIT ||  c_ct->mpthree.key.keysym.scancode == SDL_SCANCODE_ESCAPE) ? (c_ct->statement = 0) : 0;
...
{
    if (c_ct->mpthree.key.keysym.scancode == SDL_SCANCODE_DOWN || c_ct->mpthree.key.keysym.scancode == SDL_SCANCODE_UP) {
...
void    ft_event_main(t_win *main)
{
    ...
    {
        if (main->mpthree.type == SDL_QUIT ||  main->mpthree.key.keysym.scancode == SDL_SCANCODE_ESCAPE) {
            (main->statement = 0);
        }
...
void    ft_core(t_win *c_ct)
{
    ...
    c_ct->width  = ((int)(c_ct->width / c_ct->map.miniwidth) * c_ct->map.miniwidth);
    c_ct->height = ((int)(c_ct->height / c_ct->map.miniheight) *  c_ct->map.miniheight);
    c_ct->main.lotary = rand() % c_ct->main.quantity;
    ft_showmenu(c_ct);
    while (c_ct->statement) {
        c_ct->time = SDL_GetTicks();
        c_ct->screensurface = SDL_CreateRGBSurface(0, SCREEN_WIDTH, SCREEN_HEIGHT, 32, 0, 0, 0, 0);
        if (c_ct->flag_menu_game & FT_MAIN) {
            (Mix_PlayingMusic() == 0) ? Mix_PlayMusic((c_ct->music.track = Mix_LoadMUS("./music/support/main.mp3")), -1) : 0;
			ft_event_main(c_ct);
            ft_main(c_ct);
        }
        ...
        if (c_ct->flag_menu_game & FT_GAME) {
            c_ct->minimapsurface = SDL_CreateRGBSurface(0, c_ct->width, c_ct->height, 32, 0, 0, 0, 0);
            ft_new_event_player(c_ct);
            ft_game_core(c_ct);
        }
        SDL_FreeSurface(c_ct->screensurface);
        SDL_RenderClear(c_ct->gRenderer);
        SDL_RenderCopy(c_ct->gRenderer, c_ct->gTexture, NULL, NULL);//Render texture to screen
        (c_ct->gTexture != NULL) ? SDL_DestroyTexture(c_ct->gTexture) : 0;
        SDL_RenderPresent(c_ct->gRenderer);//Update screen
    }
}
